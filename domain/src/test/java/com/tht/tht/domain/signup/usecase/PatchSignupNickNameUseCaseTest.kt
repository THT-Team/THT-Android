...
internal class PatchSignupNickNameUseCaseTest {

    private lateinit var useCase: PatchSignupNickNameUseCase
    ...
    fun setupTest() {
        ...
        useCase = PatchSignupNickNameUseCase(
            repository,
            testDispatcher
        )
    }
    ...
    @Test
    fun `useCase는 매개변수 nickaname이 유효하지 않으면 InputDataInvalidateException를 Result로 래핑하여 리턴한다`() = runTest(testDispatcher) {
    ...
    fun `useCase는 매개변수 phone을 Repository의 patchSignupUser의 매개변수로 호출한다`() = runTest(testDispatcher) {
        ...
        useCase(phone,  "nickname")
        coVerify(exactly = 1) { repository.patchSignupUser(phone, any()) }
    }

    @Test
    fun `useCase는 Repository의 fetchSignupUser결과의 nickname을 변경하여 patchSignupUser의 매개변수로 호출한다`() = runTest(testDispatcher) {
        val nickname = "nickname"
        val signupUser = SignupUserModel.getFromDefaultArgument()
        coEvery { repository.fetchSignupUser(any()) } returns signupUser
        val expect = signupUser.copy(nickname = nickname)
    ...
    fun `useCase는 Repository의 patchSignupUser의 결과를 Result로 래핑해서 리턴한다`() = runTest(testDispatcher) {
        ...
        val actual = useCase("test",  "nickname").getOrNull()
        assertThat(actual)
            .isNotNull
            .isTrue
    }
    ...
    fun `useCase는 Repository의 fetchSignupUser결과가 null이면 SignupUserInvalidateException를 Result로 래핑하여 리턴한다`() = runTest(testDispatcher) {
        ...
        useCase("test",  "nickname")

        val actual = useCase("test",  "nickname")
