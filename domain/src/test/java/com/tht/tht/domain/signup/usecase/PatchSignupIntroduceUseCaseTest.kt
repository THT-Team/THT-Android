...
internal class PatchSignupIntroduceUseCaseTest {

    private lateinit var useCase: PatchSignupIntroduceUseCase
    ...
    fun setupTest() {
        ...
        useCase = PatchSignupIntroduceUseCase(
            repository,
            testDispatcher
        )
    }
    ...
    @Test
    fun `useCase는 매개변수 introduce가 유효하지 않으면 InputDataInvalidateException를 Result로 래핑하여 리턴한다`() = runTest(testDispatcher) {
    ...
    fun `useCase는 매개변수 phone을 Repository의 patchSignupUser의 매개변수로 호출한다`() = runTest(testDispatcher) {
        ...
        useCase(phone, "introduce")
        coVerify(exactly = 1) { repository.patchSignupUser(phone, any()) }
    }

    @Test
    fun `useCase는 Repository의 fetchSignupUser결과의 introduce를 변경하여 patchSignupUser의 매개변수로 호출한다`() = runTest(testDispatcher) {
        val introduceData = "introduce"
        val signupUser = SignupUserModel.getFromDefaultArgument()
        coEvery { repository.fetchSignupUser(any()) } returns signupUser
        val expect = signupUser.copy(introduce = introduceData)

        useCase("phone", introduceData)
    ...
    fun `useCase는 Repository의 patchSignupUser의 결과를 Result로 래핑해서 리턴한다`() = runTest(testDispatcher) {
        ...
        val actual = useCase("test", "introduce").getOrNull()
    ...
    fun `useCase는 Repository의 fetchSignupUser결과가 null이면 SignupUserInvalidateException를 Result로 래핑하여 리턴한다`() = runTest(testDispatcher) {
        ...
        val actual = useCase("test", "introduce")
