...
internal class PatchSignupInterestUseCaseTest {

    private lateinit var useCase: PatchSignupInterestUseCase
    private lateinit var repository: SignupRepository
    private val testDispatcher = StandardTestDispatcher(TestCoroutineScheduler())

    private val validInterestKey = listOf("key1", "key2", "key3")
    private val invalidInterestKey = emptyList<String>()


    @Before
    fun setupTest() {
        repository = mockk(relaxed = true)
        useCase = PatchSignupInterestUseCase(
            repository,
            testDispatcher
        )
    }
    ...
    @Test
    fun `useCase는 매개변수 interestKeys가 유효하지 않으면 InterestRequireSizeException를 Result로 래핑하여 리턴한다`() = runTest(testDispatcher) {
        coEvery { repository.fetchSignupUser(any()) } returns mockk()
        val actual = useCase("phone", invalidInterestKey)
    ...
    fun `useCase는 매개변수 phone을 Repository의 patchSignupUser의 매개변수로 호출한다`() = runTest(testDispatcher) {
        ...
        useCase(phone, validInterestKey)
        coVerify(exactly = 1) { repository.patchSignupUser(phone, any()) }
    }

    @Test
    fun `useCase는 Repository의 fetchSignupUser결과의 interestKeys를 변경하여 patchSignupUser의 매개변수로 호출한다`() = runTest(testDispatcher) {
        val signupUser = SignupUserModel.getFromDefaultArgument()
        coEvery { repository.fetchSignupUser(any()) } returns signupUser
        val expect = signupUser.copy(interestKeys = validInterestKey)
    ...
    fun `useCase는 Repository의 patchSignupUser의 결과를 Result로 래핑해서 리턴한다`() = runTest(testDispatcher) {
        ...
        val actual = useCase("test", validInterestKey).getOrNull()
    ...
    fun `useCase는 Repository의 fetchSignupUser결과가 null이면 SignupUserInvalidateException를 Result로 래핑하여 리턴한다`() = runTest(testDispatcher) {
        ...
        useCase("test", validInterestKey)

        val actual = useCase("test", validInterestKey)
