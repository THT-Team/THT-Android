...
class CheckAndRefreshThtAccessTokenUseCase(
    private val refreshThtAccessTokenUseCase: RefreshThtAccessTokenUseCase,
        ...
        return kotlin.runCatching {
            when (checkThtAccessTokenExpiredUseCase().getOrNull() ?: false) {
                true -> true
                    ...
                    resultRetry(refreshRetryCount) { refreshThtAccessTokenUseCase() }
                        .let { !it.getOrThrow().accessToken.isNullOrBlank() }
                }
            }
        }
    }
